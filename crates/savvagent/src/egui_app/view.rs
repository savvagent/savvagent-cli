...
fn paint_screen_overlay(state: &mut SavvagentApp, ctx: &egui::Context, palette: &Palette) {
    ...
    // borrow on `state.app` ends before the area closure. The screen's
    // `render(region)` is the only call that touches per-frame screen state,
    // and screens are owned by the stack, so the cloned outputs
    // (lines/tips/id/layout) are stable for this frame.
