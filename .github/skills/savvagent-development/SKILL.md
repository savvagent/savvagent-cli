...
[`CLAUDE.md`](../../../CLAUDE.md) is the primary conventions document — read it and follow it — but
when it disagrees with the current code (docs can lag an evolving codebase; see the Load-Bearing
Invariants note below), the code wins. [`PRD.md`](../../../PRD.md) is the product vision and scope of
...
| Conventions of record  | [`CLAUDE.md`](../../../CLAUDE.md) at the repo root is the primary conventions document — read it before any non-trivial change — but defer to the code where the two disagree (docs can lag the code; see the Load-Bearing Invariants note above). `PRD.md` is the vision/scope of record, `crates/savvagent-protocol/SPEC.md` is the wire-format spec of record, `README.md` documents developer-facing conventions. Read the code. |
...
commands. The branch name and `<scope>: <subject>` must come from your own kebab slug — validate
just the `<kebab-slug>` portion (match `^[a-z0-9]+(-[a-z0-9]+)*$`; the full branch name additionally
has an `<area>/` prefix and the full commit/PR title additionally has a `<scope>: ` prefix, neither
of which this regex covers) and never paste a raw issue title verbatim into a `git commit`
