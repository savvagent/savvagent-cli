...
- TUI editor widget follow-up (see §9): `tui-textarea` remains the prompt widget. The separate `ratatui-code-editor`-backed in-TUI viewer/editor shipped in M5/M6 but has since been removed, so no widget-consolidation follow-up remains on the roadmap.
...
- **TUI editor widget.** *Updated after M5/M6.* `tui-textarea` (the `tui-textarea-2` fork) remains the prompt-input widget. The separate `ratatui-code-editor`-backed in-TUI viewer/editor has since been removed, so there is no longer a pending single-widget consolidation question.
